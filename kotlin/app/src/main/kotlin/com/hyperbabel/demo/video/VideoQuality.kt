...
import io.agora.rtc2.video.VideoDimensions
...
object VideoQuality {
     ...
     *   everyone in the channel, not only those with a camera on: a muted
     *   camera can be turned back on at any moment. Over-counting lowers the
     *   resolution (safe); under-counting is what pushes the call into a
     *   higher tier than you declared.
     */
    fun presetForRemoteCount(remoteCount: Int): VideoPreset =
        if (remoteCount.coerceAtLeast(0) <= 1) SOLO_OR_1TO1 else GROUP

    /** The same preset shaped for the RTC SDK's encoder configuration call. */
    fun encoderForRemoteCount(remoteCount: Int): VideoEncoderConfiguration {
        val p = presetForRemoteCount(remoteCount)
        return VideoEncoderConfiguration(
            VideoDimensions(p.width, p.height),
            p.frameRate,
            // STANDARD_BITRATE lets the SDK pick the bitrate that matches the
     ...
     * tier here as well.
     */
    fun declaredQuality(): String =
        if (SOLO_OR_1TO1.pixels <= HD_BUDGET_PX) "hd" else "fhd"
     ...
     */
    fun publishResolutionFor(participantCount: Int): PublishResolution {
        val total = if (participantCount > 0) participantCount else 1
        val p = presetForRemoteCount(total - 1)
        return PublishResolution(p.width, p.height)
    }
}

/** Wire shape of the optional `publish_resolution` field. */
@kotlinx.serialization.Serializable
data class PublishResolution(val width: Int, val height: Int)
